...
| Breadcrumb links + a "Copy page" button leaked at the top of every section (react.dev Tailwind MDX) | `<article>` opens with a `flex justify-between` header row holding breadcrumbs (`[Learn React](/learn)`) and the copy-button | receipt `strip_tags`: `div[class*='justify-between']` + `button[aria-label*='Copy']`. Do NOT blanket-strip `button` — Sandpack/Prism use `<button>` for code file-tabs (App.js/index.js) whose labels are page content |
