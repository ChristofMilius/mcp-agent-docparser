...
| Blank line between **every** code line | Astro docs wrap each code line in `.ec-line`/`.line` divs; markdownify turns each div into its own paragraph | `_collapse_pre_blocks()` rebuilds such `<pre>` as one `<pre><code>` of joined plain lines (carries `data-language`), before markdownify runs |
