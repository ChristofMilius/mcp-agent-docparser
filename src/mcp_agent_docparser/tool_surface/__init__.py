...
Call auditing:
  ...
  each handler so the mcp_agent_docparser.tool_surface logger records a DEBUG
  ...
  bounded even for long markdown output.
"""
...
from mcp_agent_docparser.tool_surface import (
    misc_tools,
    parse_tools,
    probe_tools,
    receipt_tools,
...
def register_all(server, ctx) -> None:
    ...
    receipt_tools.register(server, ctx)
    parse_tools.register(server, ctx)
    probe_tools.register(server, ctx)
